...
export default function EventDetailPage() {
  ...
  const setItem = useEventBookingStore((s) => s.setItem);
  const setShow = useEventBookingStore((s) => s.setShow);
  const setSeats = useEventBookingStore((s) => s.setSeats);
  ...
  const eventTime = useMemo(
    () => (event?.startDateTime ? formatTime(event.startDateTime) : null),
    [event?.startDateTime]
  );
  const venueLabel = useMemo(() => {
    if (!event) return null;
    if (event.venue && event.city) return `${event.venue}, ${event.city}`;
    return event.venue || event.city || null;
  }, [event]);
  const venueId = event?.venueId || "1921";
  const isReady = Boolean(eventDate && eventTime && venueLabel);

  const handleBooking = () => {
    if (!event || !eventDate || !eventTime || !venueLabel) {
      toast.warning("Event details are missing.");
      return;
    }

    setItem("event", { ...event, name: event.title });
    setShow(venueId, venueLabel, eventDate, eventTime);
    setSeats([], Number(event.price || 0));
    router.push(`/events/${id}/review`);
  };
            ...
            <div
              className={`pointer-events-none absolute inset-0 ${
                dark
                  ...
                  : "bg-[radial-gradient(circle_at_top,_rgba(99,102,241,0.12),_transparent_55%)]"
              }`}
            />
              ...
              <div className="space-y-6">
                ...
                <div className="grid auto-rows-fr items-stretch gap-3 sm:grid-cols-2">
                  <div className={`h-full rounded-2xl border p-4 ${dark ? "border-zinc-800 bg-zinc-950/80" : "border-slate-200 bg-slate-50"}`}>
                    <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-indigo-500">
                      <Calendar className="h-3.5 w-3.5" />
                      Date
                    </div>
                ...
                <div className="grid auto-rows-fr items-stretch gap-3 sm:grid-cols-2">
                  <div className={`flex h-full flex-col justify-between rounded-2xl border p-4 ${dark ? "border-indigo-500/40 bg-indigo-500/10" : "border-indigo-200 bg-indigo-50"}`}>
                    <div>
                      <p className={`text-xs font-semibold uppercase tracking-[0.2em] ${dark ? "text-indigo-200" : "text-indigo-700"}`}>
                        Starting at
                      </p>
                    ...
                    <p className={`mt-3 text-xs ${dark ? "text-indigo-200/80" : "text-indigo-700/80"}`}>
