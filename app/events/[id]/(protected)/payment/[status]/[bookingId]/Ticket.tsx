...
import ResetEventStores from "../../components/ResetEventStores";
...
export default async function Ticket({ id, status, bookingId }: Props) {
  const data = await serverFetch(`/events/booking/${bookingId}`, {
    authRedirectPath: `/events/${id}/payment/${status}/${bookingId}`,
  });
  const booking = data?.booking as Booking;
  const payment = data?.payment as Payment;
  const event = (await serverFetch(`/events/${id}`, {
