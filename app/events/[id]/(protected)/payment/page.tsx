...
const PaymentPage = () => {
  ...
  const goToSuccess = (bookingId: string) => {
    startTransition(() => {
      router.replace(`/events/${item?._id}/payment/success/${bookingId}`);
    });
  };

  const goToFailed = (bookingId: string) => {
    startTransition(() => {
      router.replace(`/events/${item?._id}/payment/failed/${bookingId}`);
    });
  };
  ...
  const handlePayment = async () => {
    try {
      ...
      const orderData = await apiFetch("/events/payment/create-order", {
        method: "POST",
        headers: {
      ...
      if (method === "wallet") {
        ...
        const walletResult = await apiFetch("/events/payment/wallet-pay", {
          method: "POST",
          body: JSON.stringify({
      ...
      const eventTitle = (item as Event | null)?.title;
      const description = eventTitle || item?.name || "event tickets";
      ...
      const options = {
        ...
        handler: async function (response: {
          ...
          try {
            const verification = await apiFetch("/events/payment/verify", {
              method: "POST",
              body: JSON.stringify({
                bookingId: orderData.bookingId,
        ...
        modal: {
          ondismiss: async function () {
            ...
            try {
              await apiFetch("/events/payment/fail", {
                method: "POST",
                body: JSON.stringify({
                  bookingId: orderData.bookingId,
                }),
  ...
  return (
    <div className={`min-h-screen px-4 pb-10 pt-28 ${mode === "dark" ? "bg-zinc-950" : "bg-gradient-to-b from-slate-50 to-white"}`}>
      <CheckoutNavbar backUrl={`/events/${item?._id}/review`} />
          ...
          <div className="mt-4 space-y-3 text-sm">
            <div className={`flex items-center justify-between ${mode === "dark" ? "text-zinc-300" : "text-slate-600"}`}>
              ...
              <span className={`max-w-[65%] truncate text-right font-medium ${mode === "dark" ? "text-zinc-100" : "text-slate-900"}`}>
                {(item as Event | null)?.title || item?.name || "-"}
              </span>
            </div>
            <div className={`flex items-center justify-between ${mode === "dark" ? "text-zinc-300" : "text-slate-600"}`}>
              <span>Date</span>
