...
const TicketReview = () => {
  ...
  const bookingState = useEventBookingStore();
  const {
    item,
    seats,
    totalPrice,
  ...
  const handleProceedToPay = async () => {
    try {
      ...
      const data = await apiFetch("/events/payment/prepare", {
        method: "POST",
      ...
      router.push(`/events/${item?._id}/payment`);
    } catch (err: unknown) {
      ...
      <div className="mx-auto grid w-full max-w-5xl gap-4 lg:grid-cols-[1.25fr_0.75fr] lg:gap-5">
        <section className="space-y-4 lg:space-y-5">
          ...
          <TicketCard
            ...
            bookingStore={useEventBookingStore}
          />
