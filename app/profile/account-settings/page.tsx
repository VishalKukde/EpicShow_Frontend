    ...
    <div className="space-y-5 px-3 py-2 pb-6 select-none sm:px-4 lg:px-0">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-1 border-b pb-4 border-slate-200 dark:border-zinc-800">
        ...
        <button
          ...
          disabled={saving}
          className={`inline-flex cursor-pointer items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold transition shadow-sm ${
            dark
      ...
      >
          ...
          {user?.membership === "pro" ? (
            ...
            <span
              className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
                dark ? "bg-zinc-800 text-zinc-400" : "bg-slate-100 text-slate-600"
              }`}
            >
              Standard Account
            </span>
        ...
        <div
          className={`mt-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border p-4.5 ${
            dark ? "border-zinc-800 bg-zinc-950/60" : "border-slate-200 bg-slate-50/70"
          }`}
        >
              ...
              {loading ? (
                ...
                <div className="relative">
                  <Image
                    ...
                    width={80}
                    height={80}
                    className={`h-20 w-20 rounded-full object-cover shadow ring-4 ${
                      dark ? "ring-zinc-800" : "ring-white"
                    }`}
                  />
                  <button
                    type="button"
          ...
          {!loading && (
            <button
              type="button"
              onClick={() => setAvatarModalOpen(true)}
              className={`inline-flex shrink-0 items-center justify-center gap-1.5 rounded-xl border px-3.5 py-1.5 text-xs font-bold transition cursor-pointer ${
                dark
        ...
        <div className="mt-6 flex items-center justify-end gap-3 border-t pt-4 border-slate-100 dark:border-zinc-800">
          <button
            ...
            onClick={handleSaveProfile}
            disabled={saving}
            className={`inline-flex cursor-pointer items-center gap-2 rounded-xl px-5 py-2 text-xs font-bold transition shadow-sm ${
              dark
                ? "bg-sky-500 text-slate-950 hover:bg-sky-400 disabled:opacity-60"
                : "bg-slate-900 text-white hover:bg-slate-800 disabled:opacity-60"
            }`}
...
function Field({
  ...
  return (
    ...
    >
      <span
        className={`mb-2 flex items-center justify-between text-xs font-bold uppercase tracking-wider ${
    ...
    >
      <div
        ...
        className={`w-full max-w-lg rounded-3xl border p-6 shadow-2xl ${
          dark ? "border-zinc-800 bg-zinc-950 text-zinc-100" : "border-slate-200 bg-white text-slate-900"
        }`}
      >
        ...
        <div className="mb-6 flex justify-center">
          {selectedAvatar ? (
            <Image
              ...
              width={96}
              height={96}
              className={`h-24 w-24 rounded-full border-4 object-cover shadow-md ring-4 ${
                dark ? "border-zinc-800 ring-zinc-700" : "border-white ring-slate-100"
              }`}
            />
          ) : (
        ...
        <div
          className={`mb-5 rounded-2xl border p-3.5 ${
            dark ? "border-indigo-500/30 bg-indigo-500/10" : "border-indigo-100 bg-indigo-50"
          }`}
        >
          <div className="mb-2 flex items-center justify-between gap-3">
            ...
            <span
              className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${
                dark ? "bg-zinc-800 text-indigo-300" : "bg-white text-indigo-700 shadow-sm"
              }`}
            >
        ...
        <label
          className={`mb-5 flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed px-4 py-2.5 text-xs font-bold transition ${
            dark
        ...
        <div className="grid grid-cols-3 gap-3 sm:grid-cols-6">
          {presetAvatars.map((img) => (
            <button
              ...
              className={`overflow-hidden rounded-full border-2 transition cursor-pointer ${
                selectedAvatar === img
        ...
        <div
          className={`mt-5 flex justify-end gap-2.5 border-t pt-4 ${
