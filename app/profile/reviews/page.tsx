...
export default function ReviewPage() {
  ...
  return (
    <div className="select-none space-y-5 px-3 py-2 pb-6 sm:px-4 lg:px-0">
      ...
      <section
        className={`rounded-2xl border p-4 shadow-sm sm:p-5 ${dark ? "border-zinc-800 bg-[#18181b]" : "border-gray-200 bg-white"
        ...
        {loading ? (
          <div className="grid gap-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <div
                key={index}
                className={`h-32 animate-pulse rounded-2xl border ${dark ? "border-zinc-800 bg-zinc-800/60" : "bg-gray-100"
          ...
          <div className={`py-12 text-center ${dark ? "text-zinc-400" : "text-gray-500"} dark:text-zinc-400`}>
            ...
            {hasMore ? (
              <div className="mt-5 flex justify-center">
                <button
                  ...
                  className={`rounded-xl px-5 py-2.5 text-sm font-semibold transition cursor-pointer disabled:cursor-not-allowed disabled:opacity-60 ${dark
...
function ReviewCard({ dark, review }: { dark: boolean; review: UserReview }) {
  ...
  return (
    <article
      className={`overflow-hidden rounded-2xl border ${dark ? "border-zinc-800 bg-zinc-900/60" : "border-gray-200 bg-gray-50/80"
      ...
      <div className="grid gap-4 p-4 sm:grid-cols-[88px_1fr]">
        <div className="relative h-32 overflow-hidden rounded-xl sm:h-full">
          {movie?.imageUrl ? (
            ...
            <div className={`flex h-full items-center justify-center ${dark ? "bg-zinc-800" : "bg-gray-200"} dark:bg-zinc-800`}>
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <p className={`line-clamp-1 text-lg font-semibold ${dark ? "text-zinc-100" : "text-gray-900"} dark:text-white`}>
                {movie?.name || "Movie unavailable"}
              </p>
              <div className={`mt-1 flex flex-wrap items-center gap-2 text-xs ${dark ? "text-zinc-400" : "text-gray-500"} dark:text-zinc-400`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm font-semibold ${dark ? "text-zinc-200" : "text-gray-700"} dark:text-zinc-200`}>
                {review.rating}/5
              </span>
            </div>
          </div>

          <p className={`mt-3 text-sm leading-6 ${dark ? "text-zinc-300" : "text-gray-700"} dark:text-zinc-300`}>
...
function ReviewMeta({
  ...
  return (
    <div className={`rounded-xl border px-3 py-2 ${dark ? "border-zinc-800 bg-zinc-900/80" : "border-gray-200 bg-white"} dark:bg-zinc-900/80 dark:border-zinc-800`}>
      <p className={`flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] ${dark ? "text-zinc-500" : "text-gray-500"} dark:text-zinc-400`}>
        <Icon className="h-3.5 w-3.5" />
        {label}
      </p>
      <p className={`mt-1 line-clamp-1 text-xs font-semibold ${dark ? "text-zinc-200" : "text-gray-800"} dark:text-zinc-200`}>
        {value}
      </p>
    </div>
  );
