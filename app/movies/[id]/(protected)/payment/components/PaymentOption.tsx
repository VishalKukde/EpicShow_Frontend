...
const PaymentOption = ({
    ...
    <button
        onClick={onClick}
        disabled={disabled}
        className={`w-full flex items-center justify-between rounded-2xl border px-4 py-4 text-left transition-all duration-200 sm:px-5 ${disabled
            ? mode === "dark"
    ...
    >
        <div>
            ...
            {badge && (
                <span className={`mt-2 inline-flex rounded-full border px-2.5 py-0.5 text-xs font-medium ${mode === "dark"
        ...
        <div
            className={`h-5 w-5 rounded-full border-2 flex items-center justify-center ${active
